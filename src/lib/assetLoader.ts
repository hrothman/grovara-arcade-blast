...
/**
 * Warm the browser cache with every image the Phaser game will need
 * (enemies, products, rare products). Call this while the player is still
 * on the welcome screen so that when they hit "Play" the assets resolve
 * instantly from cache and no loading bar is shown.
 *
 * Safe to call multiple times — the work runs at most once, and a failed
 * image never blocks the result.
 */
    ...
    allAssets.map(
      asset =>
        new Promise<void>(resolve => {
          ...
          const done = () => resolve();
          img.onload = () => {
            // decode() fully warms the bitmap so Phaser's first draw is instant too.
