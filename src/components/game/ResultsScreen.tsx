...
export const ResultsScreen = () => {
  ...
  // Build compact display: top 5 + player
  const top5 = leaderboard.slice(0, 5);
  const playerEntry = playerRankIdx > 4 ? leaderboard[playerRankIdx] : null;
  const showSeparator = playerEntry !== null;

  const handleShare = () => {
    const shareText = `I scored ${gameState.totalScore.toLocaleString()} points playing Grovara Expo Hero!`;
    if (navigator.share) {
      navigator.share({
        title: 'Grovara Expo Hero',
        text: shareText,
